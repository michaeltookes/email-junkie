...
59. **Onboarding: sign-in-and-go default + guided power-user model options**
    ...
    *As Marcus, I want the app drafting for me minutes after install with nothing to configure, so that I never have to understand what an API key is; as Sam, I want a guided way to plug in my own key or local model, so that no server is ever in my loop.*
    ...
    - **Live-test findings from 56a (2026-08-20) to fold into the redesign:**
      ...
      - **Disconnect must always work.** The BYO **Disconnect** button is a no-op when managed is the active provider; it should clear the BYO key regardless of which provider is selected.
      - **Error messages must not linger or duplicate.** Sign-in errors currently persist until the next attempt and were rendered twice (`ManagedSignInControls` + the Settings section). Clear on input change / stage change; one render site only. *(Fix in progress on the `managed-inference` branch.)*
      - **Name the real problem on sign-up failure.** When Clerk returns `missing_requirements`, surface the `missing_fields` (e.g. a misconfigured password requirement) instead of "That code didn't complete sign-in." *(Fix in progress on the `managed-inference` branch.)*
