...
59. **Onboarding: sign-in-and-go default + guided power-user model options**
    ...
    *As Marcus, I want the app drafting for me minutes after install with nothing to configure, so that I never have to understand what an API key is; as Sam, I want a guided way to plug in my own key or local model, so that no server is ever in my loop.*
    - **Default path:** install → sign in / start trial → connect mailbox → drafting works. No model decisions surfaced by default.
    - **"Choose your AI" onboarding step (owner direction 2026-08-20):** the existing step stays, but is re-led by the managed option — a primary card/selection ("Sentwise AI — included with your subscription, nothing to set up", pre-selected, the sign-in/trial action lives here) with **Continue** as the only prominent button. The BYO option is present on the same screen but **visually subdued**: a small secondary link/disclosure such as "Use your own AI provider instead" that expands the current provider picker + key field + Test Connection inline. Power users find it without hunting; Marcus never has to read it.
    - **Live-test findings from 56a (2026-08-20) to fold into the redesign:**
      - **Make the active provider unmistakable.** With an account signed in and a BYO key still configured, both cards look live. Show an "Active" badge on the provider currently drafting; show "Use Sentwise AI" only when managed is *not* active (and a matching "Use this provider" on the BYO card only when it isn't).
      - **Disconnect must always work.** ~~The BYO **Disconnect** button is a no-op when managed is the active provider~~ — fixed 2026-08-20 on `managed-inference` (`disconnectLLM(provider:)` targets the displayed BYO provider).
      - **Error messages must not linger or duplicate.** Sign-in errors currently persist until the next attempt and were rendered twice (`ManagedSignInControls` + the Settings section). Clear on input change / stage change; one render site only. *(Shipped 2026-08-20 on `managed-inference`.)*
      - **Name the real problem on sign-up failure.** When Clerk returns `missing_requirements`, surface the `missing_fields` (e.g. a misconfigured password requirement) instead of "That code didn't complete sign-in." *(Shipped 2026-08-20 on `managed-inference`.)*
      - **First-time users sign up, not sign in.** Sign-in falls back to Clerk's sign-up flow on `form_identifier_not_found` (shipped 2026-08-20, `14e3ae7`); onboarding copy should say "Sign in or create your account" so new users aren't confused by "sign-in code."
      - **Google sign-in** was deferred out of 56a; add it here as the one-click path alongside email code.
